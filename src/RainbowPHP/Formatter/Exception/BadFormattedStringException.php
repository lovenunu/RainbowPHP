<?php
/*************************************************************************************/
/* This file is part of the RainbowPHP package. If you think this file is lost,      */
/* please send it to anyone kind enough to take care of it. Thank you.               */
/*                                                                                   */
/* email : bperche9@gmail.com                                                        */
/* web : http://www.benjaminperche.fr                                                */
/*                                                                                   */
/* For the full copyright and license information, please view the LICENSE.txt       */
/* file that was distributed with this source code.                                  */
/*************************************************************************************/

namespace RainbowPHP\Formatter\Exception;

/**
 * Class BadFormattedStringException
 * @package RainbowPHP\Formatter\Exception
 * @author Benjamin Perche <bperche9@gmail.com>
 */
class BadFormattedStringException extends \InvalidArgumentException
{
    public static function create($value)
    {
        return new static(sprintf("The value '%s' is not correctly formatted", $value));
    }
}
